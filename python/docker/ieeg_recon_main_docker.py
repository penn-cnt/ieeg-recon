...
def run_module2(args):
    cmd = ["python", "/code/module2.py", "-s", args.subject, "-rs", args.reference_session, "-d", args.source_directory, "-cs", args.clinical_session]
    ...
    if args.deface_outputs:
        ...
        subprocess.call(["python","/code/module2_deface_outputs.py","-s", args.subject, "-d", args.source_directory])


    if args.brain_shift:
        print("Applying brain shift correction to module 2 outputs...")
        subprocess.call(["python", "/code/brain_shift.py", "-s", args.subject, "-rs", args.reference_session, "-d", args.source_directory, "-fs", args.freesurfer_dir, "-cs", args.clinical_session])
...
def run_reports(args):
    subprocess.call(["python", "/code/create_workspace.py", "-s", args.subject, "-rs", args.reference_session, "-d", args.source_directory, "-cs", args.clinical_session])
    subprocess.call(["python", "/code/create_html.py", "-s", args.subject, "-rs", args.reference_session, "-d", args.source_directory, "-cs", args.clinical_session])

def run_mni(args):
    print('Running MNI registration, make sure Module 2 has ran already (i.e. run with -m 2 or -m -1 flags if not)')
    subprocess.call(["python", "/code/module3_mni_v2.py", "-s", args.subject, "-rs", args.reference_session, "-cs", args.clinical_session,"-d", args.source_directory])
...
def run_module3(args):
    ...
    if args.convert_atlas:
        subprocess.call(["python", "/code/module3_atlas_from_mni.py", "-s", args.subject, "-rs", args.reference_session, "-a", args.atlas_path,"-an",args.atlas_name,"-d", args.source_directory])
