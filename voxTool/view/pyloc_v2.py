...
class CloudViewer(HasTraits):
    ...
    def update_cloud(self, label):
        if self.clouds:
            if label=='_leads':
                self.clouds[label].update_colors()
            else:
...
class CloudView(object):
    ...
    def update(self):
        ...
        outline.bounds = (x.mean()-2, x.mean()+2,
                  y.mean()-2, y.mean()+2,
                  z.mean()-2, z.mean()+2)
        """ self._plot.mlab_source.reset(
            x=x.astype(int), y=y.astype(int), z=z.astype(int), scalars=np.array(len(self.get_colors(labels, x, y, z))*[0])+1) """
